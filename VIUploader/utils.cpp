...
int ShowMessage(char *title, char *format, ...)
{
	char str[4096];
	va_list vararg;
	int tBytes;
	...
	{
		POPUPDATA pi = {0};
		strncpy(pi.lptzContactName, title, sizeof(pi.lptzContactName));
		strncpy(pi.lpzText, str, sizeof(pi.lpzText));
		pi.lchIcon = hiVIUploaderIcon;
		
		return PUAddPopUp(&pi);
	}
	else{
		return MessageBoxA(0, str, title, MB_OK | MB_ICONINFORMATION);
	}
...
int GetStringFromDatabase(HANDLE hContact, char *szModule, char *szSettingName, char *szError, char *szResult, int size)
{
	...
	if (DBGetContactSettingTString(hContact, szModule, szSettingName, &dbv) == 0)
		{
			res = 0;
			mir_snprintf(szResult, size, "%s", dbv.pszVal);
			...
			if (szError)
				{
					mir_snprintf(szResult, size, "%s", szError);
				}
				else{
					szResult[0] = '\0';
				}
